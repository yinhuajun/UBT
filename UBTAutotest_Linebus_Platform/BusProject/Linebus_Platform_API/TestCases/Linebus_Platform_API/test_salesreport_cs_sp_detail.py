...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("salesreport_cs_sp_detail")


@ddt
class Salesreport_Cs_Sp_Detail(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
    ...
    def setUp(self):
        ...
        if CD.type_tid is None:
            type_tid = operation_mysql("SELECT tenant_id FROM bus_salecenter.sc_channel where type='0' LIMIT 1")[0][
                'tenant_id']
            setattr(CD, 'type_tid', type_tid)
    ...
    @data(*testdata)
    def test_salesreport_cs_sp_detail(self, datas):
