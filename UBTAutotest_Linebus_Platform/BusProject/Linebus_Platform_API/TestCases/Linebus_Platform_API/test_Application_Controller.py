...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("Application_Controller_news")


@ddt
class Application_Controller(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
    ...
    def test_linebus_a_r_add(self, datas):
        ...
        try:
            ...
            self.assertEqual(str(eval(datas['expected'])['msg']), str(res.json()['msg']))
            Result = "PASS"
        except AssertionError as e:
            Result = "FAIL"
            raise e
        finally:
            self.read_data.write_back(datas['module'], datas['id'] + 1, str(res.json()), Result)
