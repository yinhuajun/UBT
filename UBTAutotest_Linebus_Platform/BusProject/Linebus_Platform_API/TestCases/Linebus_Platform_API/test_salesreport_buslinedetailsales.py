...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("salesreport_buslinedetailsales")


@ddt
class Salesreport_Buslinedetailsales(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
    ...
    def setUp(self):
        ...
        if CD.buslineId is None:
            busline = operation_mysql(
                "SELECT bus_line_id FROM bus_linebus_engine.lb_bus_line_mapping  order by create_time desc LIMIT 1 ")
            buslineId = busline[0]['bus_line_id']
            setattr(CD, 'buslineId', buslineId)
        # bsp_user_username = bsp_user[0]["username"]
        # bsp_user_password = bsp_user[0]["password"]
    ...
    @data(*testdata)
    def test_salesreport_buslinedetailsales(self, datas):
